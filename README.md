...
Anything slow or retryable, like sending email, processing an upload, or calling
a flaky third-party API, shouldn't run inside the request. It should go on a
...
`enqueue` is a single `LPUSH`, so keep one client around and reuse it.
...
- **No in-flight tracking.** A task is popped with `BRPOP`, so if a worker
  ...
  is lost, since there's no processing set or visibility timeout to recover it. The
